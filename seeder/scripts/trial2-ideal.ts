...
// Read wanted parameters from file
const INCLUDED_TYPES = fs.readFileSync('wanted-parameters.txt', 'utf8')
  .split('\n')
  .map(line => line.trim())
...
async function fetchPlacesInGrid(resolution: number) {
  ...
  console.log(`   Using ${INCLUDED_TYPES.length} types from wanted-parameters.txt`);
  ...
  const outputPath = path.join(outputDir, `prague-r15-ideal.json`);
  ...
  reportLines.push(`# Prague Grid Scan Report — Resolution ${resolution} (Ideal Parameters)`);
  ...
  reportLines.push(`- Types used: ${INCLUDED_TYPES.length} from wanted-parameters.txt`);
  reportLines.push('');
  ...
  const reportPath = path.join(outputDir, `prague-r15-ideal-report.md`);
...
const resolution = parseInt(process.argv[2] || '15', 10);
if (isNaN(resolution) || resolution < 1) {
  console.error('Usage: tsx scripts/trial2-ideal.ts [resolution]');
  process.exit(1);
}
