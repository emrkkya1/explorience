...
export class ProximityDedupStage implements Stage {
  name = 'proximity_dedup';
  ...
  async run(places: RawPlace[], config: PipelineConfig): Promise<RawPlace[]> {
    ...
    const thresholdMeters = stageConfig?.params?.thresholdMeters ?? 50;
    ...
    for (let i = 0; i < places.length; i++) {
      ...
      for (let j = i + 1; j < places.length; j++) {
        ...
        if (distance <= thresholdMeters) {
          ...
          logger.debug('Found proximity duplicate', {
            kept: toKeep.name,
